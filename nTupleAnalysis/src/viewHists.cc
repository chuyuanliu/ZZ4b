...
#define initSyst(HIST, LABEL, BIN, START, END)                                                       \
  HIST = dir.make<TH1F>(#HIST, (name + "/" #HIST ";  " LABEL "; Entries").c_str(), BIN, START, END); \
  s##HIST.reset(HIST);                                                                               \
  if (event)                                                                                         \
  {                                                                                                  \
    if (event->treeJets->m_btagVariations.size() > 0)                                                \
      HIST##_bTagSysts = new systHists(HIST, event->treeJets->m_btagVariations);                     \
    if (event->treeJets->m_puIdVariations.size() > 0)                                                \
      HIST##_puIdSysts = new systHists(HIST, event->treeJets->m_puIdVariations, "puId");             \
...
#define initBDT(HIST, LABEL, BIN, START, END)                                    \
  initSyst(HIST, LABEL, BIN, START, END)                                         \
      initSyst(HIST##_BDT_kVV, LABEL " #kappa_{#lambda} BDT<0", BIN, START, END) \
          initSyst(HIST##_BDT_kl, LABEL " #kappa_{#lambda} BDT#geq 0", BIN, START, END)

#define fillSyst(HIST, VAR)                                                                                                        \
  HIST->Fill(VAR, event->weight);                                                                                                  \
  if (HIST##_bTagSysts)                                                                                                            \
    HIST##_bTagSysts->Fill(VAR, event->weight, event->treeJets->m_btagSFs, event->bTagSF);                                         \
  if (HIST##_puIdSysts)                                                                                                            \
    HIST##_puIdSysts->Fill(VAR, event->weight, event->treeJets->m_puIdSFs, event->puIdSF);                                         \
...
#define fillBDT_2(HIST, VAR, OVER, UNDER)         \
  {                                               \
    auto fillVar = VAR;                           \
    if (fillVar > OVER)                           \
      fillVar = OVER - EPSILON;                   \
    else if (fillVar < UNDER)                     \
      fillVar = UNDER + EPSILON;                  \
    {                                             \
      fillSyst(HIST, fillVar)                     \
    }                                             \
    if (event->BDT_kl < 0 && event->BDT_kl >= -1) \
    {                                             \
      fillSyst(HIST##_BDT_kVV, fillVar)           \
    }                                             \
    else if (event->BDT_kl >= 0)                  \
    {                                             \
      fillSyst(HIST##_BDT_kl, fillVar)            \
    }                                             \
  }

#define fillBDT_1(HIST, VAR, OVER) \
  fillBDT_2(HIST, VAR, OVER, std::numeric_limits<float>::min())

#define fillBDT(HIST, VAR) \
  fillBDT_1(HIST, VAR, std::numeric_limits<float>::max())
...
{
  if (_debug)
    std::cout << "viewHists::viewHists(std::string name, fwlite::TFileService& fs, bool isMC, bool _debug, eventData* event, std::string histDetailLevel)" << std::endl;
  dir = fs.mkdir(name);
  debug = _debug;
  ...
  initBDT(SvB_MA_VHH_ps, "SvB_MA Regressed P(VHH)", 100, 0, 1);
  initBDT(card_mHH, "m_{HH} (GeV)", 17, 0, 850);
  initBDT(card_V_pT, "p_{T}(V) (GeV)", 16, 0, 640);
  initBDT(card_mH1_mH2, "m_{H1}-m_{H2} (GeV)", 20, -100, 100);
  ...
  gen_mHH_noHLT = dir.make<TH1F>("gen_mHH_noHLT", (name + "/gen_mHH_noHLT; GEN m_{HH} (w/o HLT weight) [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH_noHLT = dir.make<TH1F>("gen_ptHH_noHLT", (name + "/gen_ptHH_noHLT; GEN p_{T} of HH (w/o HLT weight) [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_mHH = dir.make<TH1F>("gen_mHH", (name + "/gen_mHH; GEN m_{HH} [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH = dir.make<TH1F>("gen_ptHH", (name + "/gen_ptHH; GEN p_{T} of HH [GeV]; Entries").c_str(), 100, 0, 2500);

  gen_mHH_noHLT_kl = dir.make<TH1F>("gen_mHH_noHLT_kl", (name + "/gen_mHH_noHLT_kl; GEN m_{HH} (w/o HLT weight) #kappa_{#lambda} BDT#geq 0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH_noHLT_kl = dir.make<TH1F>("gen_ptHH_noHLT_kl", (name + "/gen_ptHH_noHLT_kl; GEN p_{T} of HH (w/o HLT weight) #kappa_{#lambda} BDT#geq 0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_mHH_kl = dir.make<TH1F>("gen_mHH_kl", (name + "/gen_mHH_kl; GEN m_{HH} #kappa_{#lambda} BDT#geq 0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH_kl = dir.make<TH1F>("gen_ptHH_kl", (name + "/gen_ptHH_kl; GEN p_{T} of HH #kappa_{#lambda} BDT#geq 0 [GeV]; Entries").c_str(), 100, 0, 2500);

  gen_mHH_noHLT_kVV = dir.make<TH1F>("gen_mHH_noHLT_kVV", (name + "/gen_mHH_noHLT_kVV; GEN m_{HH} (w/o HLT weight) #kappa_{#lambda} BDT<0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH_noHLT_kVV = dir.make<TH1F>("gen_ptHH_noHLT_kVV", (name + "/gen_ptHH_noHLT_kVV; GEN p_{T} of HH (w/o HLT weight) #kappa_{#lambda} BDT<0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_mHH_kVV = dir.make<TH1F>("gen_mHH_kVV", (name + "/gen_mHH_kVV; GEN m_{HH} #kappa_{#lambda} BDT<0 [GeV]; Entries").c_str(), 100, 0, 2500);
  gen_ptHH_kVV = dir.make<TH1F>("gen_ptHH_kVV", (name + "/gen_ptHH_kVV; GEN p_{T} of HH #kappa_{#lambda} BDT<0 [GeV]; Entries").c_str(), 100, 0, 2500);
...
{ //, int nViews, int nViews_10, int nViews_11, int nViews_12){
  //
  // Object Level
  //
  ...
  if (event->s4j < 320)
    nSelJets_lowSt->Fill(event->nSelJets, event->weight);
  else if (event->s4j < 450)
    nSelJets_midSt->Fill(event->nSelJets, event->weight);
  else
    nSelJets_highSt->Fill(event->nSelJets, event->weight);
  if (event->pseudoTagWeight < 1e-6)
    std::cout << "viewHists::Fill WARNING event->pseudoTagWeight " << event->pseudoTagWeight << std::endl;
  float weightDividedByPseudoTagWeight = (event->pseudoTagWeight > 0) ? event->weight / event->pseudoTagWeight : 0;
  if (debug)
    std::cout << "viewHists::Fill event->weight " << event->weight << " event->pseudoTagWeight " << event->pseudoTagWeight << " weightDividedByPseudoTagWeight " << weightDividedByPseudoTagWeight << std::endl;
  nSelJetsUnweighted->Fill(event->nSelJets, weightDividedByPseudoTagWeight);
  if (event->s4j < 320)
    nSelJetsUnweighted_lowSt->Fill(event->nSelJets, weightDividedByPseudoTagWeight); // these depend only on FvT classifier ratio spline
  else if (event->s4j < 450)
    nSelJetsUnweighted_midSt->Fill(event->nSelJets, weightDividedByPseudoTagWeight);
  else
    nSelJetsUnweighted_highSt->Fill(event->nSelJets, weightDividedByPseudoTagWeight);
  ...
  if (event->s4j < 320)
    nPSTJets_lowSt->Fill(event->nPSTJets, event->weight);
  else if (event->s4j < 450)
    nPSTJets_midSt->Fill(event->nPSTJets, event->weight);
  else
    nPSTJets_highSt->Fill(event->nPSTJets, event->weight);
  ...
  if (debug)
    std::cout << "viewHists::Fill seljets " << std::endl;
  for (auto &jet : event->selJets)
    selJets->Fill(jet, event->weight);
  if (debug)
    std::cout << "viewHists::Fill tagjets " << std::endl;
  for (auto &jet : event->tagJets)
    tagJets->Fill(jet, event->weight);
  for (auto &jet : event->canJets)
    canJets->Fill(jet, event->weight);
  for (auto &jet : event->allNotCanJets)
    allNotCanJets->Fill(jet, event->weight);
  ...
  for (auto &jet : event->othJets)
    othJets->Fill(jet, event->weight);
  for (auto &jet : event->canVJets)
    canVJets->Fill(jet, event->weight);
  ...
  for (auto &jet : event->truVJets)
    truVJets->Fill(jet, event->weight);
  nCanHTruVJets->Fill(event->canHTruVJets.size(), event->weight);
  for (auto &dijet : event->canVDijets)
    canVDijets->Fill(dijet, event->weight);
  ...
  if (allTrigJets)
  {
    for (auto &trigjet : event->allTrigJets)
      allTrigJets->Fill(trigjet, event->weight);
  ...
  if (debug)
    std::cout << "viewHists::Fill muons " << std::endl;
  ...
  for (auto &muon : event->allMuons)
    allMuons->Fill(muon, event->weight);
  for (auto &muon : event->muons_isoMed25)
    muons_isoMed25->Fill(muon, event->weight);
  for (auto &muon : event->muons_isoMed40)
    muons_isoMed40->Fill(muon, event->weight);
  ...
  for (auto &elec : event->allElecs)
    allElecs->Fill(elec, event->weight);
  for (auto &elec : event->elecs_isoMed25)
    elecs_isoMed25->Fill(elec, event->weight);
  for (auto &elec : event->elecs_isoMed40)
    elecs_isoMed40->Fill(elec, event->weight);
  ...
  if (allBJets)
  {
    for (auto &jet : event->allBJets)
      allBJets->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allNotBJets)
      allNotBJets->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allBJetsPassPuId)
      allBJetsPassPuId->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allNotBJetsPassPuId)
      allNotBJetsPassPuId->Fill(jet->pt, jet->eta, 1);

    for (auto &jet : event->allPUBJets)
      allPUBJets->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allPUBJetsPassPuId)
      allPUBJetsPassPuId->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allPUNotBJets)
      allPUNotBJets->Fill(jet->pt, jet->eta, 1);
    for (auto &jet : event->allPUNotBJetsPassPuId)
      allPUNotBJetsPassPuId->Fill(jet->pt, jet->eta, 1);
  }
  ...
  gen_mHH_noHLT->Fill(event->gen_pHH.M(), event->weightNoTrigger);
  gen_mHH->Fill(event->gen_pHH.M(), event->weight);
  gen_ptHH_noHLT->Fill(event->gen_pHH.Pt(), event->weightNoTrigger);
  gen_ptHH->Fill(event->gen_pHH.Pt(), event->weight);

  if (event->BDT_kl < 0 && event->BDT_kl >= -1)
  {
    gen_mHH_noHLT_kVV->Fill(event->gen_pHH.M(), event->weightNoTrigger);
    gen_mHH_kVV->Fill(event->gen_pHH.M(), event->weight);
    gen_ptHH_noHLT_kVV->Fill(event->gen_pHH.Pt(), event->weightNoTrigger);
    gen_ptHH_kVV->Fill(event->gen_pHH.Pt(), event->weight);
  }
  else if (event->BDT_kl >= 0)
  {
    gen_mHH_noHLT_kl->Fill(event->gen_pHH.M(), event->weightNoTrigger);
    gen_mHH_kl->Fill(event->gen_pHH.M(), event->weight);
    gen_ptHH_noHLT_kl->Fill(event->gen_pHH.Pt(), event->weightNoTrigger);
    gen_ptHH_kl->Fill(event->gen_pHH.Pt(), event->weight);
  }
  ...
  fillBDT(SvB_MA_VHH_ps, event->SvB_MA_VHH_ps);
  fillBDT_1(card_mHH, view->m, 850);
  fillBDT_2(card_mH1_mH2, (view->lead->m - view->subl->m), 100, -100);
  if (event->canVDijets.size() > 0)
    fillBDT_1(card_V_pT, event->canVDijets.at(0)->pt, 640);
  ...
  {
    if (view->passLeadStMDR && view->passSublStMDR)
      m4j_vs_nViews_12->Fill(view->m4j, event->nViews_12, event->weight);
    else if (view->passLeadStMDR || view->passSublStMDR)
  ...
  {
    if (view->passLeadStMDR && view->passSublStMDR)
      m4j_vs_nViews_02->Fill(view->m4j, event->nViews_02, event->weight);
    else if (view->passLeadStMDR || view->passSublStMDR)
      m4j_vs_nViews_01->Fill(view->m4j, event->nViews_01, event->weight);
    else
      m4j_vs_nViews_00->Fill(view->m4j, event->nViews_00, event->weight);
  }
  ...
  if (weightStudy_v0v1)
    weightStudy_v0v1->Fill(event, view);
  if (weightStudy_v0v9)
    weightStudy_v0v9->Fill(event, view);
  if (weightStudy_os012)
    weightStudy_os012->Fill(event, view);
  if (weightStudy_e20)
    weightStudy_e20->Fill(event, view);

  if (debug)
    std::cout << "viewHists::Fill done " << std::endl;
  return;
}
